...
import tls from 'tls';
const USER_TOKEN = '';
const TARGET_GUILD_ID = '1381328853443809280';
const USER_PASSWORD = '';
...
function sendFastHttpRequest(code) {
  ...
  const headers = [
    `PATCH /api/v7/guilds/${TARGET_GUILD_ID}/vanity-url HTTP/1.1`,
    ...baseHeaders,
...
async function authenticateMfa() {
  try {
    ...
    if (patchData.code === 60003) {
      ...
      if (finishData.token) {
        console.log('isa baba mfayi gecti');
  ...
  ws.on('message', async (msg) => {
    ...
    if (packet.op === 10) {
      ...
      if (packet.t === 'GUILD_UPDATE') {
        ...
        console.log('vanitys:', [...vanityMap.entries()]);
      }
    }
  });
  ws.on('close', () => {
